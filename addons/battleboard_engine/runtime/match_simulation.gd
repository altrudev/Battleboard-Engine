...
func apply_command(command: Dictionary) -> Dictionary:
	...
	if not bool(invariant["ok"]):
		return _failure("post_command_invariant:%s" % ",".join(PackedStringArray(invariant["errors"])))
	return {
		"ok": true,
...
func simulation_hash() -> String:
	...
	for profile_id in profile_ids:
		...
		for raw_status_id in (statuses[profile_id] as Dictionary).keys():
			status_ids.append(str(raw_status_id))
		status_ids.sort()
		for status_id in status_ids:
			var entry: Dictionary = statuses[profile_id][status_id]
...
func _append_event(event_type: String, payload := {}) -> void:
